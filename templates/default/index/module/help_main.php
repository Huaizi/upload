...
{if $show_sheet == 1}
	<div class="art_sheet">
		<div class="main">
			...
			<div><a href="{url channel='help' id=$item.id}" title="{$item.title}" target="_blank">{$item.title}</a><span>{$item.add_time|date_format:"%Y-%m-%d"}</span></div>
			{/foreach}
			{if !$article}<div class="not_found">暂无内容</div>{/if}
		</div>
		{if $global.cat}{$prefix = 'help/cat-'|cat:$global.cat}{else}{$prefix = 'help'}{/if}
